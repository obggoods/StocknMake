    ...
    <DialogContent className="flex h-[min(820px,calc(100dvh-1rem))] w-[calc(100%-1rem)] max-w-4xl flex-col gap-3 overflow-hidden sm:w-[calc(100%-2rem)] sm:max-w-[820px]">
      ...
      {draftPrompt ? <div className="flex flex-1 flex-col items-center justify-center gap-4 text-center"><p className="text-sm">작성 중인 정산이 있습니다.<br />이어서 작성하시겠어요?</p><div className="flex gap-2"><AppButton variant="outline" onClick={startFresh}>처음부터 작성</AppButton><AppButton onClick={continueDraft}>이어서 작성</AppButton></div></div> : confirming ? <div className="min-h-0 flex-1 space-y-4 overflow-y-auto text-sm"><div className="rounded-lg border p-4"><div>{month} · {selectedStore?.name}</div><div className="mt-2 font-semibold">판매 제품 {items.length}종 · 총 판매수량 {totalQuantity}개</div><div className="mt-3 grid gap-1 sm:max-w-sm"><div className="flex justify-between gap-4"><span>총매출</span><span className="font-semibold tabular-nums">{grossAmount.toLocaleString()}원</span></div>{commissionRate == null ? <div className="text-muted-foreground">{storeId ? "수수료율 미설정" : "입점처 선택 후 수수료를 계산합니다."}</div> : <><div className="flex justify-between gap-4 text-muted-foreground"><span>판매 수수료 {formatCommissionPercent(commissionRate * 100)}%</span><span className="tabular-nums">{commissionAmount?.toLocaleString()}원</span></div><div className="flex justify-between gap-4 border-t border-border/60 pt-1 font-semibold text-primary"><span>정산 예정액</span><span className="tabular-nums">{expectedSettlement?.toLocaleString()}원</span></div></>}</div></div><div className="space-y-2">{items.map((item) => <div key={item.product.id} className="flex justify-between gap-3"><span className="min-w-0 truncate">{item.product.name} · {item.unitPrice?.toLocaleString() ?? "-"}원 × {item.quantity}개</span><span className="shrink-0 tabular-nums">{item.gross.toLocaleString()}원</span></div>)}</div></div> : <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-hidden">
        ...
        <div className="shrink-0 space-y-3 rounded-xl border p-3 sm:p-4"><div className="text-sm font-medium">판매 제품 추가</div><div className="grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,2fr)_minmax(0,1fr)_minmax(0,1fr)] lg:items-end">
          <label className="min-w-0 space-y-1 text-sm">카테고리<Popover open={categoryPickerOpen} onOpenChange={setCategoryPickerOpen}><PopoverTrigger asChild><AppButton type="button" variant="outline" className="h-8 w-full min-w-0 justify-between px-3 text-left font-normal"><span className="truncate">{category || "전체 카테고리"}</span><ChevronsUpDown className="h-4 w-4 shrink-0 opacity-50" /></AppButton></PopoverTrigger><PopoverContent className="w-[min(320px,calc(100vw-3rem))] p-0" align="start"><Command><CommandInput placeholder="카테고리 검색..." /><CommandList className="max-h-60 overscroll-contain touch-pan-y"><CommandEmpty>카테고리가 없습니다.</CommandEmpty><CommandGroup><CommandItem value="전체 카테고리" onSelect={() => { setCategory(""); setSelectedProductId(""); setCategoryPickerOpen(false) }}><Check className={`mr-2 h-4 w-4 ${!category ? "opacity-100" : "opacity-0"}`} />전체 카테고리</CommandItem>{categories.map((value) => <CommandItem key={value} value={value} onSelect={() => { setCategory(value); setSelectedProductId(""); setCategoryPickerOpen(false) }}><Check className={`mr-2 h-4 w-4 ${category === value ? "opacity-100" : "opacity-0"}`} /><span className="truncate">{value}</span></CommandItem>)}</CommandGroup></CommandList></Command></PopoverContent></Popover></label>
          <label className="min-w-0 space-y-1 text-sm">제품<Popover open={productPickerOpen} onOpenChange={setProductPickerOpen}><PopoverTrigger asChild><AppButton type="button" variant="outline" className="h-8 w-full min-w-0 justify-between px-3 text-left font-normal" disabled={!storeId}><span className="min-w-0 truncate">{selectedProduct?.name ?? (storeId ? "제품명 검색 또는 선택" : "입점처 먼저 선택")}</span><ChevronsUpDown className="h-4 w-4 shrink-0 opacity-50" /></AppButton></PopoverTrigger><PopoverContent className="w-[min(420px,calc(100vw-3rem))] p-0" align="start"><Command><CommandInput placeholder="제품명, SKU, 바코드 검색..." /><CommandList className="max-h-60 overscroll-contain touch-pan-y"><CommandEmpty>제품이 없습니다.</CommandEmpty><CommandGroup>{selectableProducts.map((product) => { const added = quantities[product.id] !== undefined; return <CommandItem key={product.id} value={`${product.name} ${product.sku ?? ""} ${product.barcode ?? ""}`} disabled={added} onSelect={() => { if (!added) { setSelectedProductId(product.id); setProductPickerOpen(false) } }}><Check className={`mr-2 h-4 w-4 ${selectedProductId === product.id ? "opacity-100" : "opacity-0"}`} /><span className="min-w-0 flex-1 truncate">{product.name}</span>{added ? <span className="text-xs text-muted-foreground">이미 추가됨</span> : null}</CommandItem> })}</CommandGroup></CommandList></Command></PopoverContent></Popover></label>
