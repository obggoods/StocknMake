...
function keepDropdownWheelInside(event: WheelEvent<HTMLDivElement>) {
  ...
  const atTop = event.deltaY < 0 && list.scrollTop <= 0
  const atBottom = event.deltaY > 0 && list.scrollTop + list.clientHeight >= list.scrollHeight
...
export function ManualSettlementDialog(props: { open: boolean; onOpenChange: (open: boolean) => void; stores: Store[]; products: Product[]; inventory: Inventory[]; onSaved: () => Promise<void> }) {
  ...
  const [scrollAfterAdd, setScrollAfterAdd] = useState(false)
  ...
  const [draftPrompt, setDraftPrompt] = useState<SettlementDraft | null>(null)
  ...
  useEffect(() => {
    if (props.open) {
      ...
      void supabase.auth.getUser().then(({ data }) => {
        ...
        if (!userId) { setDraftReady(true); return }
        try {
          const raw = localStorage.getItem(draftKey(userId))
          const draft = raw ? JSON.parse(raw) as SettlementDraft : null
          const valid = draft && draft.savedAt > Date.now() - 30 * 24 * 60 * 60 * 1000 && /^\d{4}-(0[1-9]|1[0-2])$/.test(draft.periodMonth) && props.stores.some((store) => store.id === draft.marketplaceId) && draft.products.every((item) => props.products.some((product) => product.id === item.productId) && parseQuantity(item.quantity) > 0 && parseUnitPrice(item.unitPrice) != null)
          if (valid) setDraftPrompt(draft)
          else if (raw) localStorage.removeItem(draftKey(userId))
        } catch { localStorage.removeItem(draftKey(userId)) }
        setDraftReady(true)
      })
      return
    }
    setStoreId(""); setMonth(previousMonth()); setCategory(""); setSelectedProductId(""); setAddQuantity("1")
    setQuantities({}); setUnitPrices({}); setItemOrder([]); setScrollAfterAdd(false); setCategoryPickerOpen(false); setStorePickerOpen(false); setProductPickerOpen(false); setConfirming(false); setCommissionRate(null); setDraftPrompt(null); setDraftReady(false); setAuthUserId("")
  }, [props.open])

  useEffect(() => {
    if (!props.open || !draftReady || !authUserId) return
    const hasDraft = Boolean(storeId || itemOrder.length || month !== previousMonth())
    const key = draftKey(authUserId)
    const timer = window.setTimeout(() => {
      if (!hasDraft) { localStorage.removeItem(key); return }
      localStorage.setItem(key, JSON.stringify({ savedAt: Date.now(), marketplaceId: storeId, periodMonth: month, products: itemOrder.map((productId) => ({ productId, quantity: quantities[productId] ?? "", unitPrice: unitPrices[productId] ?? "" })) }))
    }, 250)
    return () => window.clearTimeout(timer)
  }, [props.open, draftReady, authUserId, storeId, month, itemOrder, quantities, unitPrices])

  const startFresh = () => { if (authUserId) localStorage.removeItem(draftKey(authUserId)); setDraftPrompt(null); setStoreId(""); setMonth(previousMonth()); setQuantities({}); setUnitPrices({}); setItemOrder([]) }
  const continueDraft = () => {
    if (!draftPrompt) return
    setStoreId(draftPrompt.marketplaceId); setMonth(draftPrompt.periodMonth); setItemOrder(draftPrompt.products.map((item) => item.productId)); setQuantities(Object.fromEntries(draftPrompt.products.map((item) => [item.productId, item.quantity]))); setUnitPrices(Object.fromEntries(draftPrompt.products.map((item) => [item.productId, item.unitPrice]))); setDraftPrompt(null)
  }
  ...
  const items = useMemo(() => itemOrder.flatMap((id) => { const product = eligibleProducts.find((candidate) => candidate.id === id); if (!product) return []; const sold = parseQuantity(quantities[id] ?? ""); const unitPrice = parseUnitPrice(unitPrices[id] ?? ""); return sold && sold > 0 ? [{ product, quantity: sold, unitPrice, gross: sold * (unitPrice ?? 0) }] : [] }), [eligibleProducts, itemOrder, quantities, unitPrices])
  ...
  const addProduct = () => {
    ...
    setUnitPrices((previous) => ({ ...previous, [selectedProduct.id]: String(Number(selectedProduct.price ?? 0)) }))
    setItemOrder((previous) => [...previous, selectedProduct.id])
  ...
  const next = () => {
    ...
    if (items.some((item) => item.unitPrice == null)) return toast.error("판매 단가를 올바르게 입력해주세요.")
    setConfirming(true)
  }
  const save = async () => {
    if (saving) return
  ...
  return <Dialog open={props.open} onOpenChange={(open) => !saving && props.onOpenChange(open)}>
    <DialogContent className="flex h-[min(820px,calc(100dvh-1rem))] w-[calc(100%-1rem)] max-w-4xl flex-col gap-3 overflow-hidden sm:w-[calc(100%-2rem)] sm:max-w-[820px]">
      ...
      {draftPrompt ? <div className="flex flex-1 flex-col items-center justify-center gap-4 text-center"><p className="text-sm">작성 중인 정산이 있습니다.<br />이어서 작성하시겠어요?</p><div className="flex gap-2"><AppButton variant="outline" onClick={startFresh}>처음부터 작성</AppButton><AppButton onClick={continueDraft}>이어서 작성</AppButton></div></div> : confirming ? <div className="min-h-0 flex-1 space-y-4 overflow-y-auto text-sm"><div className="rounded-lg border p-4"><div>{month} · {selectedStore?.name}</div><div className="mt-2 font-semibold">판매 제품 {items.length}종 · 총 판매수량 {totalQuantity}개</div><div className="mt-3 grid gap-1 sm:max-w-sm"><div className="flex justify-between gap-4"><span>총매출</span><span className="font-semibold tabular-nums">{grossAmount.toLocaleString()}원</span></div>{commissionRate == null ? <div className="text-muted-foreground">{storeId ? "수수료율 미설정" : "입점처 선택 후 수수료를 계산합니다."}</div> : <><div className="flex justify-between gap-4 text-muted-foreground"><span>판매 수수료 {formatCommissionPercent(commissionRate * 100)}%</span><span className="tabular-nums">{commissionAmount?.toLocaleString()}원</span></div><div className="flex justify-between gap-4 border-t border-border/60 pt-1 font-semibold text-primary"><span>정산 예정액</span><span className="tabular-nums">{expectedSettlement?.toLocaleString()}원</span></div></>}</div></div><div className="space-y-2">{items.map((item) => <div key={item.product.id} className="flex justify-between gap-3"><span className="min-w-0 truncate">{item.product.name} · {item.unitPrice?.toLocaleString() ?? "-"}원 × {item.quantity}개</span><span className="shrink-0 tabular-nums">{item.gross.toLocaleString()}원</span></div>)}</div></div> : <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-hidden">
        ...
        <div className="shrink-0 space-y-3 rounded-xl border p-3 sm:p-4"><div className="text-sm font-medium">판매 제품 추가</div><div className="grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,2fr)_minmax(0,1fr)_minmax(0,1fr)] lg:items-end">
          <label className="min-w-0 space-y-1 text-sm">카테고리<Popover open={categoryPickerOpen} onOpenChange={setCategoryPickerOpen}><PopoverTrigger asChild><AppButton type="button" variant="outline" className="h-8 w-full min-w-0 justify-between px-3 text-left font-normal"><span className="truncate">{category || "전체 카테고리"}</span><ChevronsUpDown className="h-4 w-4 shrink-0 opacity-50" /></AppButton></PopoverTrigger><PopoverContent className="w-[min(320px,calc(100vw-3rem))] p-0" align="start"><Command><CommandInput placeholder="카테고리 검색..." /><CommandList onWheel={keepDropdownWheelInside} className="max-h-60 overscroll-contain touch-pan-y"><CommandEmpty>카테고리가 없습니다.</CommandEmpty><CommandGroup><CommandItem value="전체 카테고리" onSelect={() => { setCategory(""); setSelectedProductId(""); setCategoryPickerOpen(false) }}><Check className={`mr-2 h-4 w-4 ${!category ? "opacity-100" : "opacity-0"}`} />전체 카테고리</CommandItem>{categories.map((value) => <CommandItem key={value} value={value} onSelect={() => { setCategory(value); setSelectedProductId(""); setCategoryPickerOpen(false) }}><Check className={`mr-2 h-4 w-4 ${category === value ? "opacity-100" : "opacity-0"}`} /><span className="truncate">{value}</span></CommandItem>)}</CommandGroup></CommandList></Command></PopoverContent></Popover></label>
          <label className="min-w-0 space-y-1 text-sm">제품<Popover open={productPickerOpen} onOpenChange={setProductPickerOpen}><PopoverTrigger asChild><AppButton type="button" variant="outline" className="h-8 w-full min-w-0 justify-between px-3 text-left font-normal" disabled={!storeId}><span className="min-w-0 truncate">{selectedProduct?.name ?? (storeId ? "제품명 검색 또는 선택" : "입점처 먼저 선택")}</span><ChevronsUpDown className="h-4 w-4 shrink-0 opacity-50" /></AppButton></PopoverTrigger><PopoverContent className="w-[min(420px,calc(100vw-3rem))] p-0" align="start"><Command><CommandInput placeholder="제품명, SKU, 바코드 검색..." /><CommandList onWheel={keepDropdownWheelInside} className="max-h-60 overscroll-contain touch-pan-y"><CommandEmpty>제품이 없습니다.</CommandEmpty><CommandGroup>{selectableProducts.map((product) => { const added = quantities[product.id] !== undefined; return <CommandItem key={product.id} value={`${product.name} ${product.sku ?? ""} ${product.barcode ?? ""}`} disabled={added} onSelect={() => { if (!added) { setSelectedProductId(product.id); setProductPickerOpen(false) } }}><Check className={`mr-2 h-4 w-4 ${selectedProductId === product.id ? "opacity-100" : "opacity-0"}`} /><span className="min-w-0 flex-1 truncate">{product.name}</span>{added ? <span className="text-xs text-muted-foreground">이미 추가됨</span> : null}</CommandItem> })}</CommandGroup></CommandList></Command></PopoverContent></Popover></label>
