...
export function ManualSettlementDialog(props: { open: boolean; onOpenChange: (open: boolean) => void; stores: Store[]; products: Product[]; storeProductStates: StoreProductState[]; onSaved: () => Promise<void> }) {
  ...
  const items = useMemo(() => itemOrder.flatMap((id) => { const product = eligibleProducts.find((candidate) => candidate.id === id); if (!product) return []; const sold = parseQuantity(quantities[id] ?? "") ?? 0; const unitPrice = parseUnitPrice(unitPrices[id] ?? ""); return [{ product, quantity: sold, unitPrice, gross: sold * (unitPrice ?? 0) }] }), [eligibleProducts, itemOrder, quantities, unitPrices])
  ...
  const next = () => {
    ...
    if (items.some((item) => item.quantity < 1)) return toast.error("판매 수량은 1개 이상 입력해주세요.")
